...
angular
    ...
    .factory('AudioEngine', ['OSC', 'AMP', 'FTR', '$window', function(Oscillator, Amp, Filter, $window) {
        ...
        function _registerPianoSamples(){
            for(var i = 48; i < 85; i++){
                // var url = "/assets/ff-0" + i + '.wav'
                loadAudio(i, $window.pianoNoteSamplesSrc['note'+ i])
            }
        ...
        function loadAudio(note, url) {
            ...
            request.onload = function () {
                self.ctx.decodeAudioData(request.response, function (buffer) {
                    ...
                    $window.pianoNoteSamplesBuffer['buffer'+ note] = buffer
        ...
        function _wire(Analyser) {
            ...
            if(Analyser) {
                ...
                // self.analyser.connect(self.ctx);
            } else {
        ...
        function _noteOn(note, velocity) {
            // var sample = $window.sampleMap['key' + note];
            // btn[sample - 1].classList.add('active');

            // map each key to a noteSample
 
            var source = self.ctx.createBufferSource()
            source.buffer = $window.pianoNoteSamplesBuffer['buffer'+ note];
            // source.connect(self.analyser);
            source.connect(self.ctx.destination);
            source.start();

            $window.peer2.send(note)
            // InBox.value = InBox.value + 'ME:  ' + Note + '\r';
            ...
            // self.amp.setVolume(0,0, self.settings.attack);
        }
